...
## v0.1.3の追加負荷試験と既知の問題（2026-09-23）

RTX830・RTX1210で、既定の接続数・問い合わせ数制限を維持した負荷試験を行いました。[試験報告](docs/load-test-v0.1.3.md)と[測定結果CSV](docs/load-test-v0.1.3.csv)を公開しています。

**約4,000件のAレコードを含む約64KBの応答で、単発の問い合わせでも応答送信前に接続が閉じる問題を両機種で確認しました。v0.1.3の配布物では未修正です。** 開発版では、応答の準備完了時から既定2秒の送信期限を計算するよう修正し、両機種で単発5件ずつの成功を確認しました。新版のReleaseはまだ公開していません。[既知の問題と開発版の対応](docs/known-issues.md)
...
Yamaha RTXの内蔵UDP DNSと静的ホスト登録を維持し、LuaでTCP DNSを補完します。開発時の実機検証対象は **RTX830 Rev.15.02.33** と **RTX1210 Rev.14.01.42**、どちらも整数版Lua 5.1.5（機能1.08）です。**RTX1300 Rev.23.00.19については、協力者からv0.1.2の`rtx-dns.lua`での動作確認報告をいただいています。** v0.1.3の実機検証には含めていません。RTX810は未検証です。[動作確認一覧](docs/validation.md#実機)
...
**v0.1.3をRTX830 Rev.15.02.33・RTX1210 Rev.14.01.42で測定しました。** LAN内の合成上流DNSを使い、キャッシュなし・問い合わせごとに新しいTCP接続を作る条件で、次の各30秒の試験は予定した全件が正常応答でした。

| 応答サイズ（単一TXTレコード） | RTX830 | RTX1210 |
|---|---|---|
| 512バイト | 250クエリ/秒、7,500 / 7,500件成功 | 100クエリ/秒、3,000 / 3,000件成功 |
| 65,535バイト | 40クエリ/秒、1,200 / 1,200件成功 | 40クエリ/秒、1,200 / 1,200件成功 |

64個の送信元IPを使い、配布版のIP当たり4接続・20クエリ/秒・バースト40件、全体32接続などの制限は維持しました。上流へのTCP接続は再利用しています。機種共通の最大性能や長時間の保証値ではなく、今回全件成功した試験条件です。同じサイズでもレコード数が多い応答では、上記の未修正問題が発生します。

より高い負荷、継続接続、キャッシュ、CPU・メモリ、RTX1210のLAN1受信オーバーフローを含む結果は[追加負荷試験報告](docs/load-test-v0.1.3.md)に記載しています。

公開前に実施した別条件の試験や、`dns server select`対応前のRTX830での100クエリ/秒・1時間の測定は[検証範囲と過去の測定](docs/validation.md)に残しています。v0.1.3で1時間の試験は再実施していません。
