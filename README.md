...
> **v0.9.9候補版は公開準備中です。** 機種ごとのインターフェース名とIPv6のみのDNS規則への対応を追加し、実機検証を行いました。制限を緩めた過負荷試験での再起動は既知の制約として記録し、配布版の既定制限を維持します。現在の安定版はv0.1.4です。[負荷試験記録](docs/load-test-v0.9.9.md)・[v0.9.9の変更内容](docs/releases/v0.9.9.md)・[対応機種の管理表](docs/compatibility.md)
...
64個の送信元IPを使い、配布版のIP当たり4接続・20クエリ/秒・バースト40件、全体32接続などの制限は維持しました。上流へのTCP接続は再利用しています。機種共通の最大性能や長時間の保証値ではなく、v0.1.3で全件成功した試験条件です。v0.1.4では機能回帰試験を行い、この負荷条件は再測定していません。v0.9.9候補版では、制限を緩めた過負荷試験でRTX830の自己再起動が1回発生しました。既定制限を維持した4台の追加試験では再起動は観測していません。[v0.9.9の負荷試験記録](docs/load-test-v0.9.9.md)。上記の過去版の測定値をv0.9.9の性能として扱いません。
