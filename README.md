...
**v0.1.3のソースでRTX830 Rev.15.02.33・RTX1210 Rev.14.01.42の短時間負荷試験を行いました。** 配布版と同じ送信元IP制限では、両機種とも512バイト・10クエリ/秒を45秒（450件）、65,535バイト・5クエリ/秒を45秒（225件）、すべて正常受信しました。

さらに、同じソースの試験設定で送信元IPごとの上限を32接続・1,000クエリ/秒・バースト1,000件へ変更し、以下を確認しました。**配布版の既定値は4接続・20クエリ/秒・バースト40件のままです。**

| 条件（LAN内の合成上流DNS、キャッシュなし） | RTX830 | RTX1210 |
|---|---|---|
| 512バイト・100クエリ/秒・180秒 | 18,000 / 18,000件成功 | 18,000 / 18,000件成功 |
| 65,535バイト・20クエリ/秒・60秒 | 1,200 / 1,200件成功 | 1,200 / 1,200件成功 |

上流TCP接続は各機種で1本を再利用し、通信エラー・負荷生成側の取りこぼしは0件でした。これは短時間の確認であり、最大性能・同時接続容量・長時間の安定性を測定したものではありません。WAN越しの上流遅延や応答サイズでも性能は変わります。

RTX830での100クエリ/秒・1時間の過去実績は`dns server select`対応前の実装の結果で、v0.1.3では再実施していません。[今回の測定条件・応答時間・CPUと過去の測定](docs/validation.md)
