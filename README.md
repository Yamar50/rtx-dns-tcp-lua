...
**v0.1.2の既知の問題：** キャッシュ対象の判定、複数要求後のTCP半閉鎖、DNSクラスの制限に関する3点を確認しました。発生条件と影響は[既知の問題](docs/known-issues.md)をご覧ください。修正版を準備しています。
...
Yamaha RTXの内蔵UDP DNSと静的ホスト登録を維持し、LuaでTCP DNSを補完します。開発時の実機検証対象は **RTX830 Rev.15.02.33** と **RTX1210 Rev.14.01.42**、どちらも整数版Lua 5.1.5（機能1.08）です。**RTX1300 Rev.23.00.19についても、協力者から実機での動作確認報告をいただいています。** RTX810は未検証です。[動作確認一覧](docs/validation.md#実機)
...
**RTX830・RTX1210で実機検証を行い、RTX1300 Rev.23.00.19は協力者から動作確認の報告をいただいています。** 秒間クエリ数の参考値はRTX830での測定結果です。
