...
![スクリプトあり：UDP応答のサイズ超過通知を受けたクライアントがTCPで再問い合わせし、Luaが大きなDNS応答を返す](https://raw.githubusercontent.com/Yamar50/rtx-dns-tcp-lua/e4adb49ae063f0936611de9a33f1e889e553bdb8/docs/images/dns-tcp-with-script.png)
...
![スクリプトなし：TCPで再問い合わせしてもYAMAHA RTXは処理できず、タイムアウトなどのエラーになる](https://raw.githubusercontent.com/Yamar50/rtx-dns-tcp-lua/e4adb49ae063f0936611de9a33f1e889e553bdb8/docs/images/dns-tcp-without-script.png)
...
ルーターからHTTPS接続できる場合は、[この版を導入するコマンド](https://github.com/Yamar50/rtx-dns-tcp-lua/blob/main/docs/installer.md#version-v014)を管理者コンソールへ貼り付けて実行できます。**取得する本体はv0.1.4に固定**され、SHA256照合・差し替え・起動確認まで自動で行います。自動起動の登録は開始時の`yes`／`no`で指定します。`yes`は現在のルーター設定全体を保存します。

インストーラーを手動転送する場合：[rtx-dns-install-v0.1.4.lua](https://github.com/Yamar50/rtx-dns-tcp-lua/releases/download/v0.1.4/rtx-dns-install-v0.1.4.lua)。ルーター上の保存名と実行方法は上記の手順を参照してください。
