...
### v0.9.9専用インストーラー（Pre-release）

ルーターからHTTPS接続できる場合は、[この版を導入するコマンド](https://github.com/Yamar50/rtx-dns-tcp-lua/blob/main/docs/installer.md#version-v099)を管理者コンソールへ貼り付けて実行できます。**取得する本体はv0.9.9に固定**され、SHA256照合・差し替え・起動確認まで自動で行います。自動起動の登録は開始時の`yes`／`no`で指定します。`yes`は現在のルーター設定全体を保存します。

インストーラーを手動転送する場合：[rtx-dns-install-v0.9.9.lua](https://github.com/Yamar50/rtx-dns-tcp-lua/releases/download/v0.9.9/rtx-dns-install-v0.9.9.lua)。ルーター上の保存名と実行方法は上記の手順を参照してください。

### 本体を手動で転送する場合

USBメモリやmicroSDカード、SFTP等でインストールします。機種が備える転送方法を使用してください。以下はUSBメモリを使う例です。

1. 配布ファイル（[rtx-dns.lua][download-lua]と[SHA256SUMS][download-sha]）をダウンロードし、SHA-256を確認します。LuaファイルをUSBメモリのルートへ保存し、ルーターへ接続します。
