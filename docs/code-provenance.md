...
追加確認時点の本体SHA-256（現在版のハッシュではありません）：
...
PP・DHCPの取得状態を読む`dns_runtime.lua`を追加し、DNS選択・AAAA応答加工・接続とキャッシュの更新処理を変更した。これらのソースコード・テスト・ドキュメントもOpenAI Codexで生成した。現在のビルドは本プロジェクトの7モジュールと設定を結合し、外部コードを取得・同梱しない。

今回の公開前確認は、実装と対応方針の照合、テスト、配布ファイルの一致確認、公開対象への環境情報・認証情報の混入確認である。上記の外部公開ソースとの検索・比較を今回の追加・変更部分すべてに再実施したものではない。
